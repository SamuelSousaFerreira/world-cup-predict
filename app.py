...
with tab_jogo:
    ...
    if res:
        ...
        fav_idx = int(np.argmax(ens))
        fav_label = [h, "empate", a][fav_idx]
        fav_result = ["H", "D", "A"][fav_idx]
        ...
        with right:
            # Placar coerente com o favorito do ENSEMBLE (evita a contradição
            # "favorito X, mas placar de empate").
            gh, ga, ps = res["best_by_result"][fav_result]
            if fav_result == "D":
                st.markdown("**Placar sugerido** (favorito: empate)")
                st.markdown(f"### {h} {gh} × {ga} {a}")
            else:
                st.markdown(f"**Placar sugerido** (favorito: {fav_label})")
                st.markdown(f"### {h} {gh} × {ga} {a}")
            st.caption(f"placar mais provável com {fav_label} ({ps*100:.1f}% dos jogos)")

            st.markdown("**Placares mais prováveis (Poisson)**")
            icon = {"H": h, "D": "empate", "A": a}
            sc = pd.DataFrame(
                [{"Placar": f"{h} {i} × {j} {a}", "Resultado": icon[lab], "Prob.": p}
                 for i, j, p, lab in res["top_scores"]]
            ).set_index("Placar")
            st.dataframe(sc.style.format({"Prob.": "{:.1%}"}), width="stretch")
            st.caption(
                "ℹ️ O placar individual mais provável costuma ser um empate porque "
                "as vitórias se espalham por muitos placares diferentes. Veja a leitura "
                "por resultado abaixo."
            )

            # Leitura do Poisson por resultado (transparência: o placar vem deste
            # modelo, que pode discordar do ensemble sobre o favorito).
            pp = res["poisson_probs"]
            st.markdown("**Poisson — por resultado**")
            st.caption(
                f"{h} {pp[0]*100:.0f}% · empate {pp[1]*100:.0f}% · {a} {pp[2]*100:.0f}%"
            )
            if np.argmax(pp) != fav_idx:
                st.caption(
                    f"⚠️ O Poisson favorece **{[h,'empate',a][int(np.argmax(pp))]}**, "
                    f"mas o ensemble favorece **{fav_label}** — por isso o placar "
                    "isolado pode parecer contraditório."
                )
